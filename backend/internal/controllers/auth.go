...
type LoginRequest struct {
    ...
    Senha string `json:"senha"` // <-- O "S" tem que ser MAIÚSCULO aqui!
}
...
func Login(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			fmt.Println("ERRO NA BUSCA DO BANCO:", err)
			http.Error(w, `{"erro": "E-mail ou senha incorretos"}`, http.StatusUnauthorized)
			return
		}

		fmt.Println("E-mail encontrado:", usuario.Email)
		fmt.Println("Hash que veio do banco:", usuario.SenhaHash)
		fmt.Println("Senha que veio do Front-end:", req.Senha)
		...
		if err != nil {
            fmt.Println("ERRO DO BCRYPT:", err)
			http.Error(w, `{"erro": "E-mail ou senha incorretos"}`, http.StatusUnauthorized)
			return
		}
